...
function export_sql {
    SQLFILE=$BACKUP_PREFIX_DB"-database_$DB_CHARSET.sql.gz"
    echo "Dumping database to $SQLFILE"

    DOCKER_CMD="nice -n 19 mysqldump --single-transaction \
    ...
    echo $DOCKER_CMD
    docker exec -it xlp_mediawiki $DOCKER_CMD
...
BACKUP_PREFIX_IMG="./mountpoint/backup_restore/mediawiki/$PREFIX"
BACKUP_PREFIX_DB="/mnt/backup_restore/mariadb/$PREFIX"

echo $BACKUP_PREFIX_IMG
echo $BACKUP_PREFIX_DB
